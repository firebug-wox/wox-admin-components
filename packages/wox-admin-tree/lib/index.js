...
var _initialiseProps = function _initialiseProps() {
  ...
  this.dragStart = function (id, dom, e) {
    ...
    _this2._start = true;
    window.addEventListener('mousedown', _this2.down);
    window.addEventListener('mousemove', _this2.drag);
    window.addEventListener('mouseup', _this2.dragEnd);
  };

  this.down = function (e) {
    _this2.mouseDown = { x: e.clientX, y: e.clientY };
  };

  this.drag = function (e) {
    console.log(_this2.mouseDown);
    if (e.clientX === _this2.mouseDown.x && e.clientY === _this2.mouseDown.y) {
      return false;
    }
    if (_this2._start) {
      _this2.setState({
    ...
    if (diffY < 0) {
      // up
      if (index.prev) {
        var above = tree.getIndex(index.prev);
        ...
        var _above = tree.getNodeByTop(index.top - index.height);
        if (_above && _above.parent === index.parent) {
          if (_above.children && _above.children.length && !_above.node.collapsed) {
            newIndex = tree.move(index.id, _above.id, 'before');
          } else {
            newIndex = tree.move(index.id, _above.id, 'before');
          }
        }
      ...
      if (index.next) {
        ...
        if (_below && _below.parent === index.parent) {
  ...
  this.dragEnd = function (e) {
    ...
    if (_this2.up) {
      _this2.change(_this2.state.tree, e);
      _this2.up = false;
    }
  ...
  this.change = function (tree, target) {
    _this2._updated = true;
    if (_this2.props.onChange) _this2.props.onChange(tree.obj, target);
  };
